#ifndef MOCHIMOCHI_NAROW_HPP_
#define MOCHIMOCHI_NAROW_HPP_
...
public :
  NAROW(const std::size_t dim, const double r)
    : kDim(dim),
      kR(r),
...
public :
  bool update(const Eigen::VectorXd& feature, const int label) {
    ...
    const auto gamma = (confidence > 1 / kR) ? confidence / (kR * confidence - 1.0) : std::numeric_limits<double>::infinity();
    const auto beta = 1.0 / (confidence + gamma);
    const auto alpha = std::max(0.0, 1.0 - label * margin) * beta;

    functions::enumerate(feature.data(), feature.data() + feature.size(), 0,
                       [&](const int index, const double value) {
                         const auto v = _covariances[index] * value;
                         _means[index] += alpha * label * value;
